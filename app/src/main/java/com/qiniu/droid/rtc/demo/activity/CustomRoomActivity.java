...
public class CustomRoomActivity extends AppCompatActivity implements QNRTCEngineEventListener, ControlFragment.OnCallEvents, SensorEventListener {
    private static final String TAG = "CustomRoomActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_custom_muti_track_room);
    ...
    private void initLocalTrackInfoList() {
        ...
        switch (mCaptureMode) {
            case Config.CUSTOM_CAPTURE:
                ...
                Log.e(TAG, "initLocalTrackInfoList: getTrackId " + mLocalVideoTrack.getTrackId());
                if (mCameraRenderer != null) {
                    mCameraRenderer.setQNTrackInfo(mLocalVideoTrack);
                }
            ...
            public void onConfirmClicked() {
                ...
                if (!mMergeLayoutConfigView.isStreamingEnabled()) {
                    ...
                    if (mIsMergeJobStreaming) {
                        ...
                        mEngine.stopMergeStream(mCurrentMergeJob == null ? null : mCurrentMergeJob.getMergeJobId());
                        mIsMergeJobStreaming = false;
                        ToastUtils.s(CustomRoomActivity.this, "停止合流！！！");
                    } else {
                        ToastUtils.s(CustomRoomActivity.this, "未开启合流，配置未生效！！！");
                    }
                    if (mPopWindow != null) {
                        mPopWindow.dismiss();
                    }
                ...
                if (mIsForwardJobStreaming && !mMergeLayoutConfigView.isCustomMergeJob()) {
                    Utils.showAlertDialog(CustomRoomActivity.this, getString(R.string.create_merge_job_warning));
    ...
    private void setMergeStreamLayouts() {
        ...
        ToastUtils.s(CustomRoomActivity.this, "已发送合流配置，请等待合流画面生效");
    }
    ...
    public void onKickedOut(String userId) {
        ToastUtils.s(CustomRoomActivity.this, getString(R.string.kicked_by_admin));
        finish();
    }
    ...
    public void onCreateMergeJobSuccess(String mergeJobId) {
        updateSerialNum();
        ToastUtils.s(CustomRoomActivity.this, "合流任务 " + mergeJobId + " 创建成功！");
        setMergeStreamLayouts();
    ...
    public void onCreateForwardJobSuccess(String forwardJobId) {
        ...
        ToastUtils.s(CustomRoomActivity.this, "单路转推任务 " + forwardJobId + " 创建成功！");
    ...
    public void onError(int errorCode, String description) {
        ...
        switch (errorCode) {
            ...
            case QNErrorCode.ERROR_TOKEN_EXPIRED:
                ...
                mRoomToken = QNAppServer.getInstance().requestRoomToken(CustomRoomActivity.this, mUserId, mRoomId);
                mEngine.joinRoom(mRoomToken);
                break;
    ...
    public void onCallStreamingConfig() {
        if (!mIsAdmin) {
            ToastUtils.s(CustomRoomActivity.this, "只有 \"admin\" 用户可以开启合流转推！！！");
            return;
        }
        //配置页
    ...
    public void onToggleForwardJob() {
        if (!mIsAdmin) {
            ToastUtils.s(CustomRoomActivity.this, "只有 \"admin\" 用户可以开启单流转推！！！");
            return;
        }
        if (!mIsForwardJobStreaming) {
            ...
            mIsForwardJobStreaming = false;
            mControlFragment.updateForwardJobText(getString(R.string.forward_job_btn_text));
            ToastUtils.s(CustomRoomActivity.this, "已停止 id=" + mForwardJob.getForwardJobId() + " 的单流转推！！！");
        }
    }
