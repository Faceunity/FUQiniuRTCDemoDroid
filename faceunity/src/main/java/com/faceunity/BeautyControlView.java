...
import static com.faceunity.entity.Filter.sFilterName;
...
public class BeautyControlView extends FrameLayout {
    ...
    private void faceunityDefaultParam() {
        if (mOnFaceUnityControlListener != null) {
            mOnFaceUnityControlListener.onALLBlurLevelSelected(mFaceBeautyALLBlurLevel);
            mOnFaceUnityControlListener.onBeautySkinTypeSelected(mFaceBeautyType);
            mOnFaceUnityControlListener.onBlurLevelSelected(mFaceBeautyBlurLevel >= 1000 ? 0 : mFaceBeautyBlurLevel);
            mOnFaceUnityControlListener.onColorLevelSelected(mFaceBeautyColorLevel >= 1000 ? 0 : mFaceBeautyColorLevel);
            mOnFaceUnityControlListener.onRedLevelSelected(mFaceBeautyRedLevel >= 1000 ? 0 : mFaceBeautyRedLevel);
            mOnFaceUnityControlListener.onBrightEyesSelected(mBrightEyesLevel >= 1000 ? 0 : mBrightEyesLevel);
            mOnFaceUnityControlListener.onBeautyTeethSelected(mBeautyTeethLevel >= 1000 ? 0 : mBeautyTeethLevel);
            if (mFaceBeautyFaceShape == 4) {
                mOnFaceUnityControlListener.onEnlargeEyeSelected(mFaceBeautyEnlargeEye >= 1000 ? 0 : mFaceBeautyEnlargeEye);
            } else {
                mOnFaceUnityControlListener.onEnlargeEyeSelected(mFaceBeautyEnlargeEye_old >= 1000 ? 0 : mFaceBeautyEnlargeEye_old);
            }
            if (mFaceBeautyFaceShape == 4) {
                mOnFaceUnityControlListener.onCheekThinSelected(mFaceBeautyCheekThin >= 1000 ? 0 : mFaceBeautyCheekThin);
            } else {
                mOnFaceUnityControlListener.onCheekThinSelected(mFaceBeautyCheekThin_old >= 1000 ? 0 : mFaceBeautyCheekThin_old);
            }
            mOnFaceUnityControlListener.onChinLevelSelected(mChinLevel >= 1000 ? 0.5f : mChinLevel);
            mOnFaceUnityControlListener.onForeheadLevelSelected(mForeheadLevel >= 1000 ? 0.5f : mForeheadLevel);
            mOnFaceUnityControlListener.onForeheadLevelSelected(mForeheadLevel >= 1000 ? 0.5f : mForeheadLevel);
            mOnFaceUnityControlListener.onThinNoseLevelSelected(mThinNoseLevel >= 1000 ? 0 : mThinNoseLevel);
        }
    }

    //重新设置道具参数
    private void setProp() {
        if (mEffectPositionSelect == 0) {
            return;
        }
        mEffectRecyclerAdapter.notifyDataSetChanged();
