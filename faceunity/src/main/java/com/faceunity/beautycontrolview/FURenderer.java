...
//          faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], "blur_blend_ratio", 1);
    ...
    public int onDrawFrameFBO(byte[] mData, int texId, int texWidth, int texHeight) {
        if (!isActive) {
            return texId;
        }
        ...
        if (isFront) {
            Matrix.scaleM(transformMatrix, 0, 1, -1, 1);//正式摄像头
            ...
            fuTex = onDrawFrameDoubleInput(mData, fboTex[0], texWidth, texHeight);
        } else {
            fuTex = onDrawFrameDoubleInput(mData, texId, texWidth, texHeight);
        }
